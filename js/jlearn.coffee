...
(CardCtrl = (jlearn, timeout, filter) ->
	...
	jlearn.saveAll = () ->
		console.log 'save'
		localStorage.userData = filter('json')(jlearn.userData)
	...
	jlearn.nextCard = (index) ->
		...
		if flip
			...
			jlearn.previousCard = jlearn.currentCard or {source: ''}
			...
			if jlearn.currentCard.source.match /\.(jpg|png|jpeg|gif)/
				...
				if jlearn.currentCard.source.indexOf('http://cdn.rszr.co') is -1
			...
			if jlearn.flipped
				...
				jlearn.backCard = cardHtml jlearn.currentCard
			else
				jlearn.frontCard = cardHtml jlearn.currentCard
				jlearn.backCard = cardHtml jlearn.previousCard

	jlearn.answer = (event) ->
		if event.keyCode is 32 and jlearn.input.length <= 0
			jlearn.answer = {
				status: 'learn'
	...
	jlearn.cardContainerClass = () ->
		return 'image' if jlearn.currentCard.isImage
		if jlearn.currentCard.source.length > 3 or jlearn.currentCard.target.length > 3 then 'small' else 'big'

	jlearn.getClass = (card) ->
		if jlearn.getStats(card) >= 50 then 'success' else (if card.success < card.error then 'error' else '')

	jlearn.getStats = (card) ->
