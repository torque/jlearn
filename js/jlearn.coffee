...
CardCtrl = ($scope, $timeout, $filter) ->
	...
	if typeof($scope.deck.fuzzy) is 'boolean'
	...
	$scope.nextCard = (index) ->
		...
		if $scope.repeatLastCard
			$scope.repeatLastCard = false
			flip = false
		else if typeof index isnt 'undefined'
	...
	$scope.check = () ->
		$scope.currentCard.success = 0 unless $scope.currentCard.success
		$scope.currentCard.error = 0 unless $scope.currentCard.error

		target = $scope.currentCard.target.toLowerCase()
		target = target.latinise() if $scope.fuzzy

		if $scope.input.length >= 1
			$scope.hint = ''
			if target.indexOf($scope.input.toLowerCase()) is 0
				$scope.inputClass = 'ok' 
			else
				if $scope.currentCard.hint
					$scope.hint = $scope.currentCard.hint
				else if $scope.currentCard.target.length is 1
		...
		if $scope.input.length is $scope.currentCard.target.length
			if $scope.input.toLowerCase() is target
				$scope.answer = {
					status: 'success'
					card:   $scope.currentCard
				}
	...
	$scope.cardContainerClass = () ->
		return 'image' if $scope.currentCard.isImage
		return if $scope.currentCard.source.length > 3 or $scope.currentCard.target.length > 3 then 'small' else 'big'

	$scope.getClass = (card) ->
		return if $scope.getStats(card) >= 50 then 'success' else (if card.success < card.error then 'error' else '')

	$scope.getStats = (card) ->
		res = Math.floor(card.success / (card.error + card.success)*100)
		return if isNaN(res) then 0 else res

	$('#input').focus()
	$('#input').keyup $.proxy($scope.answer, $scope)

	$scope.nextCard unless $scope.random then 0 else undefined
